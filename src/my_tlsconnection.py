...
class TLSConnection(TLSRecordLayer):
    ...
    def _handshakeClientAsyncHelper(self, srpParams, certParams, anonParams,
        ...
        if ext and ext.version > (3, 3):
            ...
            if result in ["finished", "resumed_and_finished"]:
                ...
                self._clientHello = clientHello.random
                return
            else:
    ...
    def _clientSendClientHello(self, settings, session, srpUsername,
        ...
        if next((i for i in settings.versions if i > (3, 3)), None):
            # if we have a eorm Post Handshake Authentication
    ...
    def my_handshakeServer(self, verifierDB=None,
    ...
    def _handshakeServerAsyncHelper(self, verifierDB,
        ...
        (clientHello, version, cipherSuite, sig_scheme, privateKey,
            cert_chain) = result
        # For the sake of the analysis we need to save clientHello.
...
#pylint: enable = undefined-loop-variable
    ...
    def _server_select_certificate(self, settings, client_hello,
        ...
        # gquetel
        alt_certs = []
        for vh in settings.virtual_hosts:
            for accepted_sni in vh.hostnames:
                if(accepted_sni.decode() == client_hello.server_name.decode()):
                    for i in vh.keys:
                        alt_certs.append((X509CertChain(i.certificates), i.key))
     
        certs = [(cert, key)
                 for cert, key in chain(alt_certs,[(cert_chain, private_key)])]
